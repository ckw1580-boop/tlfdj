...
namespace ElectricalSim
{
    public sealed class TrainingSceneBootstrap : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            controller.Initialize(deviceViews, cameraController, wireRoot, ui.Mode, ui.Task, ui.Description, ui.Status, ui.Instrument, wireMaterial);
            BindUi(ui);
        }
        ...
        private void CreateEnvironment()
        {
            ...
            CreateCube("Cabinet", new Vector3(0f, 1.65f, 0.2f), new Vector3(3.1f, 3.3f, 0.42f), new Color(0.055f, 0.065f, 0.07f));
            for (var row = 0; row < 5; row++)
                CreateCube("DIN_Rail_" + row, new Vector3(0f, 0.55f + row * 0.55f, -0.04f), new Vector3(2.72f, 0.075f, 0.08f), new Color(0.68f, 0.72f, 0.73f));
        ...
        private void CreateMotor(string id, string label, Vector3 position)
        {
            ...
            if (original != null)
            {
                root = Instantiate(original, position, Quaternion.identity);
        ...
        private void CreatePorts(ElectricalDeviceView view, Transform parent, IReadOnlyCollection<string> ports, Vector3 bounds)
        {
            ...
            for (var index = 0; index < list.Count; index++)
            {
                ...
                var portObject = CreatePrimitive(PrimitiveType.Sphere, "Port", parent, new Vector3(x, y, -bounds.z * 0.68f - 0.025f), Vector3.one * 0.055f, new Color(0.12f, 0.86f, 0.36f));
        ...
        private HudReferences CreateHud()
        {
            ...
            var references = new HudReferences { Canvas = canvas, Top = top, Right = right, Mode = mode, Task = task, Description = description, Status = status, Instrument = instrument };
        ...
        private void BindUi(HudReferences ui)
        {
            ...
            SetRect(previous.GetComponent<RectTransform>(), new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(18f, -258f), new Vector2(175f, -208f));
            var next = Button("NextTask", ui.Right.transform, "下一项 ▶", controller.NextTask);
            SetRect(next.GetComponent<RectTransform>(), new Vector2(1f, 1f), new Vector2(1f, 1f), new Vector2(-175f, -258f), new Vector2(-18f, -208f));
            var reference = Button("Reference", ui.Right.transform, "加载标准接线", controller.LoadReferenceWiring);
            SetRect(reference.GetComponent<RectTransform>(), new Vector2(0f, 1f), new Vector2(1f, 1f), new Vector2(18f, -324f), new Vector2(-18f, -270f));
            var submit = Button("Submit", ui.Right.transform, "提交：拓扑 + 动作验收", controller.SubmitTask, new Color(0.04f, 0.58f, 0.78f));
            SetRect(submit.GetComponent<RectTransform>(), new Vector2(0f, 1f), new Vector2(1f, 1f), new Vector2(18f, -390f), new Vector2(-18f, -336f));
