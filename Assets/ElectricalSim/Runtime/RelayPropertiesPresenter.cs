...
{
    public sealed class RelayPropertiesPresenter : MonoBehaviour
    {
        private SimulationController controller;
        private IntermediateRelayView view;
        ...
        public void Initialize(SimulationController source, Canvas canvas, Font font)
        {
            ...
            button.onClick.AddListener(() => controller.SelectRelay(null));

            var scrollObject = new GameObject("RelayScroll", typeof(RectTransform), typeof(Image), typeof(ScrollRect));
        ...
        public void Show(IntermediateRelayView selected)
        {
            view = selected;
            gameObject.SetActive(view != null);
            if (view == null) return;
            transform.SetAsLastSibling();
            title.text = view.Runtime.DeviceId + " · 中间继电器属性";
            Refresh();
        ...
        {
            if (view == null) return;
            var text = controller.DescribeRelay(view);
