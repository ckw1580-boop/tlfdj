...
{
    public sealed class ContactorPropertiesPresenter : MonoBehaviour
    {
        private SimulationController controller;
        private ContactorView view;
        ...
        public void Initialize(SimulationController source, Canvas canvas, Font font)
        {
            ...
            button.onClick.AddListener(() => controller.SelectContactor(null));

            var scrollObject = new GameObject("ContactorScroll", typeof(RectTransform), typeof(Image), typeof(ScrollRect));
            ...
            hint.text = "滚动查看全部端子 · 线圈接线控制吸合，点击仅查看属性";
            gameObject.SetActive(false);
        }

        public void Show(ContactorView selected)
        {
            view = selected;
            gameObject.SetActive(view != null);
            if (view == null) return;
            transform.SetAsLastSibling();
            title.text = view.Definition.Id + (view.IsRear ? " · 背部交流接触器属性" : " · 交流接触器属性");
            Refresh();
        ...
        {
            if (view == null) return;
            var text = controller.DescribeContactor(view);
